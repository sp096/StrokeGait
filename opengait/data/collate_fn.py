...
class CollateFn(object):
    ...
    def __call__(self, batch):
        ...
        for bt in batch:
            ...
            # bt[1] can be [identity_id, class_name, view] when data_structure == 'class'
            # or [identity_id, type, view] when data_structure == 'identity'.
            # We try to find a matching label in self.label_set. Prefer bt[1][0],
            # otherwise fall back to bt[1][1] if present.
            label_key = bt[1][0]
            if label_key not in self.label_set:
                if len(bt[1]) > 1 and bt[1][1] in self.label_set:
                    label_key = bt[1][1]
                else:
                    raise ValueError(f"Label {bt[1][0]} is not in list")
            labs_batch.append(self.label_set.index(label_key))
