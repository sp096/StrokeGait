...
class RandomSpatialCropPad(object):
    """Randomly crop a small spatial region and pad back to the original size.

    This keeps the tensor shape unchanged while adding mild spatial translation
    and crop perturbation. It is intended for silhouette/binary inputs.
    """

    def __init__(self, prob=0.5, crop_ratio=(0.9, 1.0)):
        self.prob = prob
    ...
    def __call__(self, seq):
        ...
        h, w = seq.shape[-2:]
        ratio = random.uniform(self.crop_ratio[0], self.crop_ratio[1])
        ...
        cropped = seq[..., top:top + crop_h, left:left + crop_w]
...
class RandomTemporalJitter(object):
    """Lightly perturb temporal positions without changing sequence length."""

    def __init__(self, prob=0.5, max_offset=2):
        self.prob = prob
    ...
    def __call__(self, seq):
        ...
        frame_num = seq.shape[0]
        if frame_num <= 1 or self.max_offset == 0:
            return seq

        base_idx = np.arange(frame_num)
        offsets = np.random.randint(-self.max_offset, self.max_offset + 1, size=frame_num)
        jittered_idx = np.clip(base_idx + offsets, 0, frame_num - 1)
        jittered_idx = np.maximum.accumulate(jittered_idx)
        jittered_idx[-1] = frame_num - 1
        return seq[jittered_idx]


class RandomFrameDrop(object):
    """Randomly replace a few frames with neighboring frames.

    This mimics temporal missing frames while keeping the sequence length fixed.
    """

    def __init__(self, prob=0.5, drop_ratio=(0.05, 0.15)):
        self.prob = prob
    ...
    def __call__(self, seq):
        ...
        ratio = random.uniform(self.drop_ratio[0], self.drop_ratio[1])
        drop_num = max(1, int(round(frame_num * ratio)))
        drop_num = min(drop_num, frame_num - 1)
        drop_idx = np.random.choice(frame_num, size=drop_num, replace=False)

        aug_seq = seq.copy()
        for idx in drop_idx:
            if idx == 0:
                repl_idx = 1
            elif idx == frame_num - 1:
                repl_idx = frame_num - 2
            else:
                repl_idx = idx - 1 if random.random() < 0.5 else idx + 1
            aug_seq[idx] = seq[repl_idx]
        return aug_seq


class RandomGaussianNoise(object):
    """Add mild Gaussian noise to normalized silhouettes."""

    def __init__(self, prob=0.5, std=0.01, clip_min=0.0, clip_max=1.0):
        self.prob = prob
        self.std = std
    ...
    def __call__(self, seq):
        ...
        noise = np.random.normal(0.0, self.std, size=seq.shape).astype(np.float32)
        seq = seq.astype(np.float32) + noise
        return np.clip(seq, self.clip_min, self.clip_max)
