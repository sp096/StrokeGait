...
class BaseModel(MetaModel, nn.Module):
    ...
    def get_loader(self, data_cfg, train=True):
        ...
        # If dataset is organized by class (data_structure == 'class'),
        # use class-level label set for collate so that labels become class indices
        # (0..num_classes-1). Otherwise use identity-level label_set.
        label_set_for_collate = dataset.label_set
        try:
            if data_cfg.get('data_structure', 'identity') == 'class':
                # types_list holds the class names when data_structure == 'class'
                label_set_for_collate = dataset.types_set
        except Exception:
            label_set_for_collate = dataset.label_set
        ...
        loader = tordata.DataLoader(
            ...
            collate_fn=CollateFn(label_set_for_collate, sampler_cfg),
    ...
    def _load_ckpt(self, save_name):
        ...
        if not load_ckpt_strict:
            # Only keep keys that exist in current model and have matching shapes.
            cur_state = self.state_dict()
            intersect_keys = set(model_state_dict.keys()).intersection(set(cur_state.keys()))
            matched_keys = []
            mismatched_keys = []
            filtered_state = {}
            for k in intersect_keys:
                v = model_state_dict[k]
                if k in cur_state and hasattr(cur_state[k], 'size') and v.size() == cur_state[k].size():
                    filtered_state[k] = v
                    matched_keys.append(k)
                else:
                    mismatched_keys.append(k)

            # Log restored params and skipped mismatched shapes
            self.msg_mgr.log_info("-------- Restored Params List (matched) --------")
            self.msg_mgr.log_info(sorted(matched_keys))
            if mismatched_keys:
                self.msg_mgr.log_warning("-------- Skipped Params (shape mismatch) --------")
                self.msg_mgr.log_warning(sorted(mismatched_keys))

            # Load only filtered state dict (non-strict to allow missing keys)
            self.load_state_dict(filtered_state, strict=False)
        else:
            # strict loading: attempt to load full checkpoint and raise on mismatch
    ...
    def inputs_pretreament(self, inputs):
        ...
        # Force numpy arrays to float32 to avoid large float64 allocations
        # (np.asarray defaults can upcast to float64 depending on transform outputs).
