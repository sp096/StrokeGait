    ...
    def log_training_info(self):
        ...
        for i, (k, v) in enumerate(self.info_dict.items()):
            ...
            if finite_mask.any():
                ...
                value = float(finite_vals.sum() / finite_vals.size)
                if not finite_mask.all():
                    self.log_warning(
                        f"Non-finite values found in scalar/{k}. Using finite mean for logging.")
            else:
                value = float('nan')
                self.log_warning(
                    f"All values are non-finite in scalar/{k}. Logging NaN.")
            string += ", {0}={1:.4f}".format(k, value, end=end)
