...
def compute_ACC_mAP(distmat, q_pids, g_pids, q_views=None, g_views=None, rank=1):
    ...
    for q_idx in range(num_q):
        ...
        # 检查gallery是否为空
        if len(q_idx_glabels) == 0:
            # 如果gallery为空，无法进行匹配，将此probe计为失败
            all_ACC.append(0.0)  # 匹配失败
            # 不增加num_valid_q，因为无法计算AP
            continue
...
def evaluate_many(distmat, q_pids, g_pids, q_camids, g_camids, max_rank=50):
    ...
    # 修改：允许所有query都没有正确匹配的情况
    # assert num_valid_q > 0, "Error: all query identities do not appear in gallery"
    if num_valid_q == 0:
        # 如果所有query都没有正确匹配，返回全0的结果
        all_cmc = np.zeros(max_rank)
        mAP = 0.0
        mINP = 0.0
        return all_cmc, mAP, mINP
